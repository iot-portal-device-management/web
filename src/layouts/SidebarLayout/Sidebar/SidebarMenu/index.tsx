...
const renderSidebarMenuItems = ({ items, path }: RenderSidebarMenuItemsProps): JSX.Element => (
  <SubMenuWrapper>
    {items.reduce((ev, item) =>
        reduceChildRoutes({ ev, item, path }),
      [] as JSX.Element[])}
  </SubMenuWrapper>
);

const reduceChildRoutes = ({ ev, item, path }: ReduceChildRoutesProps): Array<JSX.Element> => {
  const key = item.name;
  const exactMatch = path === item.link;

  if (item.items) {
    const partialMatch = pathToRegexp(item.link, [], { sensitive: true, end: false }).test(path);
